...
export default async function KiosksPage() {
  ...
  const rows = await listKiosks(db);
  return (
    <AppShell
      title="Mặt bằng"
      action={
        canEditVehicle(user.role) ? (
          <Link href="/mat-bang/them" className="rounded-full bg-amber-400 px-3 py-2 text-sm font-bold text-[#0F4C5C]">
            Thêm ô
          </Link>
        ) : null
      }
    >
      <ul className="space-y-3">
        {rows.map((row) => {
          ...
          return (
            <li key={row.id}>
              <Link href={`/mat-bang/${row.id}`} className="block rounded-2xl bg-white p-4 ring-1 ring-[#0F4C5C]/10">
                <p className="text-2xl font-black">{row.code}</p>
                <p className="text-base">{row.location ?? "—"}</p>
                {contract ? (
                  <>
      ...
      {rows.length === 0 ? <p className="rounded-2xl bg-white p-6 text-center">Chưa có ô / ki-ốt.</p> : null}
    </AppShell>
  );
}
